...
class Erasenet(nn.Module):
    def __init__(self, n_in_channel=3, tmim=False, **kwards):
        super(Erasenet, self).__init__()
        self.tmim=tmim
       ...
       # self.aspp = ASPP(512, 256)
        ...
        if self.tmim:
            self.task_prompt1 = nn.Embedding(2, 512)
            self.task_prompt2 = nn.Embedding(2, 4*cnum)
    ...
    def forward(self, x, prompt=None):
       ...
       # import pdb;pdb.set_trace()
        ...
        x = self.conv2(x)
        if self.tmim:
            prompt1 = (self.task_prompt1(prompt)*(1-prompt).unsqueeze(1)).unsqueeze(2).unsqueeze(3)
            x = x + prompt1
        ...
        if self.tmim:
            prompt2 = (self.task_prompt2(prompt)*(1-prompt).unsqueeze(1)).unsqueeze(2).unsqueeze(3)
            x = x + prompt2
        ...
        # Output Projection
        out = self.coarse_convn(x)
        return out


class ErasenetTMIM(BaseTMIM):
    def __init__(self, *args, **kwargs):
        super(ErasenetTMIM, self).__init__()
        self.model = Erasenet(*args, **kwargs)


class ErasenetSTR(BaseSTR):
    def __init__(self, *args, **kwargs):
        super(ErasenetSTR, self).__init__()
        self.model = Erasenet(*args, **kwargs)